...
DEPLOY=$(find "$ROOT_MNT/ostree/deploy/default/deploy" -maxdepth 1 -type d -name '*.0' -print -quit 2>/dev/null || true)
if [ -z "$DEPLOY" ]; then
...
if [ -f "$ESP_MNT/loader/entries/ostree-2.conf" ]; then
    ...
    first_entry_path=$(find "$ESP_MNT/loader/entries" -maxdepth 1 -name '*.conf' -print -quit)
